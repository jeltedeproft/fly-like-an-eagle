...
export class Simulation{
 ...
 step(input={},dt=STEP){if(!['starting','ramp','air','ground'].includes(this.state))return;const sub=Math.min(.05,dt),stats=this.vehicle.stats;if(this.state==='starting'){this.startTimer=Math.max(0,this.startTimer-sub);if(this.startTimer<=1e-6){this.startTimer=0;this.state='ramp';this.goTimer=.6;this.vx=7.4*stats.launch*(1+(this.levels.launch||0)*.03)*(this.parts.nose?1.2:1)}}else if(this.state==='ramp'){this.goTimer=Math.max(0,this.goTimer-sub);const slope=(groundAt(this.x+.05)-groundAt(this.x-.05))/.1;this.vx+=(-slope*11.5+1.4)*sub;const nextX=this.x+this.vx*sub;if(nextX>=RAMP_END){this.x=RAMP_END;this.y=16;this.angle=Math.atan(.42);this.state='air';this.grounded=false;this.vy=Math.max(13.5,this.vx*.44);this.spin=-.08}else{this.x=nextX;this.y=groundAt(this.x)+1;this.angle=Math.atan(slope)}}
