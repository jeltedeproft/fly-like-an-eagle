...
export class Simulation{
 ...
 step(input={},dt=STEP){if(!['starting','ramp','air','ground'].includes(this.state))return;const sub=Math.min(.05,dt),stats=this.vehicle.stats,ramp=this.facilities.ramp||0,terrain=x=>groundAt(x,ramp);if(this.state==='starting'){this.startTimer=Math.max(0,this.startTimer-sub);if(this.startTimer<=1e-6){this.startTimer=0;this.state='ramp';this.goTimer=.6;this.vx=4.2}}else if(this.state==='ramp'){this.goTimer=Math.max(0,this.goTimer-sub);const slope=(terrain(this.x+.05)-terrain(this.x-.05))/.1,progress=clamp((this.x+64)/(RAMP_END+64),0,1),curve=progress*progress*(3-2*progress),tune=(1+(this.levels.launch||0)*.03)*(this.parts.nose?1.2:1),takeoffSpeed=Math.max(14,32*stats.launch*tune*this.powerMultiplier*(1+ramp*.08)),desiredSpeed=4.2+(takeoffSpeed-4.2)*curve;this.vx=Math.max(this.vx+Math.max(0,-slope*11.5+1.4)*tune*sub,desiredSpeed);const nextX=this.x+this.vx*sub;if(nextX>=RAMP_END){this.x=RAMP_END;this.y=terrain(RAMP_END-.001)+1;this.angle=Math.atan(.42+ramp*.085);this.state='air';this.grounded=false;this.vy=Math.max(13.5+ramp*1.8,this.vx*(.44+ramp*.022));this.spin=-.08}else{this.x=nextX;this.y=terrain(this.x)+1;this.angle=Math.atan(slope)}}
 else if(this.state==='air'){const control=(2.5+(this.levels.control||0)*.1)*stats.control*(this.parts.tailwing?1.25:1);this.spin+=(Number(Boolean(input.left))-Number(Boolean(input.right)))*control*sub;this.spin*=Math.pow(.985,sub/STEP);this.angle+=this.spin*sub;let thrust=0;if(input.boost&&this.boost>0){thrust=5.5*this.powerMultiplier;this.boost=Math.max(0,this.boost-sub)}const aero=(this.levels.aero||0)*.004;this.vx+=(thrust-this.vx*(.018-aero*.45)*stats.drag)*sub;this.vy+=(-9.2-this.vy*(.018+aero)*stats.drag)*sub;this.x+=this.vx*sub;this.y+=this.vy*sub;this.maxDistance=Math.max(this.maxDistance,this.x-RAMP_END);if(this.y<=terrain(this.x)+.8)this.land()}
