...
export class Simulation{
 ...
 step(input={},dt=STEP){if(!['ramp','air','ground'].includes(this.state))return;const sub=Math.min(.05,dt);if(this.state==='ramp'){const slope=(groundAt(this.x+.05)-groundAt(this.x-.05))/.1;this.vx+=(-slope*11.5+1.4)*sub;const nextX=this.x+this.vx*sub;if(nextX>=RAMP_END){this.x=RAMP_END;this.y=16;this.angle=Math.atan(.42);this.state='air';this.grounded=false;this.vy=Math.max(13.5,this.vx*.44);this.spin=-.08}else{this.x=nextX;this.y=groundAt(this.x)+1;this.angle=Math.atan(slope)}}
