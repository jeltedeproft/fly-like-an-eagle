...
public class FlyLikeAnEagle extends ApplicationAdapter {
    ...
    private void controls(){
        ...
        boolean left=Gdx.input.isKeyPressed(Input.Keys.LEFT)||Gdx.input.isKeyPressed(Input.Keys.A),right=Gdx.input.isKeyPressed(Input.Keys.RIGHT)||Gdx.input.isKeyPressed(Input.Keys.D);
        if(Gdx.input.isTouched()){touchLeft=Gdx.input.getX()<Gdx.graphics.getWidth()/2;touchRight=!touchLeft;left|=touchLeft;right|=touchRight;}
        if(!launched||groundContacts>0)return;
        float maxSpin=1.7f+controlLevel*.22f;
        float targetSpin=left&&!right?maxSpin:right&&!left?-maxSpin:0f;
        float current=sled.getAngularVelocity();
        float response=3.4f+controlLevel*.45f;
        float next=current+(targetSpin-current)*Math.min(1f,response*STEP);
        sled.setAngularVelocity(MathUtils.clamp(next,-maxSpin,maxSpin));
    }
